...
ts = data_processor.get_parameter(['temperature', 'salinity'], instrument_index=[1, 2, 4])
...
temp_sensors = data_processor.get_parameter(['temperature'], instrument_index=[1, 2, 3, 4, 5, 6, 7])
...
colorPurple = '#a674c0'
colorPink = '#e786cb'
col = [colorBlueBuetiful, colorOrangeBuetiful, colorGreenBuetiful, colorRedBuetiful, colorPurple, colorPink]
...
for i in range(countSen):
    elev = elev_Arr[:, i]
    # elev = np.array((temp_sensors[temp_sensors['depth']  == depths[i]])['den'])
    elev = ksi[:, i]
    # elev[np.abs(elev) > 1e2] = np.nan

    # xnew = np.linspace(0, 1, elev.shape[0])
    # elev = elev[~np.isnan(elev)]
    # xold = np.linspace(0, 1, elev.shape[0])
    # elev = interp1d(xold, elev)(xnew)
    
    # freq, sp_sum = spectrPlot(eps[5:-5], dT = (5/60), N=len(eps)-10, axPlot=ax, col='k', timeWin = 24*30*3, alpha = 0.25)
    ...
    # ax.plot(TimeTSP[50:-50], elev[50:-50], c = col[i], alpha = 0.4)
    # elev = np.convolve(elev, np.ones(winAvSp) / winAvSp, mode='same')
    ax.plot(freq[winAvSp//2:-winAvSp//2], sp_sum[winAvSp//2:-winAvSp//2], c=col[i], lw = 2, label = str(new_depths[i]) + ' m')
    # ax.plot(TimeTSP[50:-50], elev[50:-50], c = col[i])
...
#     # ax[0].plot([1/(t*3600), 1/(t*3600)], [1e-12, 10], 'k')
    # ax.axvline(1/(t * 3600), color = 'k')
# ax.axvline(f_, color = 'k')

ax.set_ylim([5e2, 4e7])
ax.set_xlim([1/(48*3600), 1/(2*3600)])

# spectrPlot(Ucr, dT = (5/60), N=len(Ucr), axPlot=ax[1], col = 'k')
ax.set_yscale('log')
ax.set_xscale('log')
...
plt.tight_layout()
plt.savefig("Spectrum_fig/Ksi_M4_spectrum.png", dpi = 1000)
...
plt.tight_layout()
plt.savefig('Spectrum_fig/SPE_M4_12_profile.png', dpi = 500, transparent=1)
...
plt.tight_layout()
plt.savefig('Spectrum_fig/SPE_M4_24_profile.png', dpi = 500, transparent=1)
plt.show()
